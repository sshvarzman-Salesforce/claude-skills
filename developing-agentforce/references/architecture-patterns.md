...
## Sequential Multi-Turn Interview (step-variable routing)

When the agent must walk the user through an ordered sequence of steps/questions over many turns — each answer validated before advancing, and later steps sometimes depending on earlier answers — drive routing with a single **step variable** instead of letting the LLM re-classify intent every turn.

```
variables:
	interview_step: mutable string = "permission"

start_agent router:
	reasoning:
		instructions: ->
			if @variables.interview_step == "permission":
				transition to @subagent.permission
			if @variables.interview_step == "eligibility":
				transition to @subagent.eligibility
			if @variables.interview_step == "end":
				transition to @subagent.end_interview

subagent permission:
	reasoning:
		instructions: ->
			| Ask whether the customer is legally permitted to work in the region.
			# only advance once the answer is captured and valid
			if @variables.has_permission == True:
				set @variables.interview_step = "eligibility"
			else:
				set @variables.interview_step = "end"
```

Principles:
- **One responsibility per subagent** — each validates exactly one answer, then sets `interview_step` to the next step.
- **The subagent owns completion** — it stays on its step (asking follow-ups) until the answer is acceptable; only then does it advance the variable.
- **Explicit, readable step names** (`"permission"`, `"eligibility"`, `"end"`) — not numeric codes.
- Use this when the sequence is long, answers must be validated, or the next step depends on the previous answer. For a single one-time prerequisite (e.g. verify identity before anything else), use the simpler **Verification Gate** pattern instead.
