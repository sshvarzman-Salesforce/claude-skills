...
|---------|------|---------|
| 0.4.9 | 2026-07-21 | **Agent Script pattern-doc gap fill** (from official ascript-patterns docs): Added list-variable operations to `agent-script-core-language.md` §6 (0-based indexing, `len()`, no-`for`-loop index+increment iteration, lists can't be slot-filled). Added `( )` grouping guidance for mixed `and`/`or` and an `is None` vs `== ""` distinction callout to §4. Added slot-fill caveat (`...` only for top-level LLM-exposed reasoning-action inputs, not `run`/chained) to §10. Added **Sequential Multi-Turn Interview (step-variable routing)** pattern to `architecture-patterns.md`. Filtering (`available when`) and transitions already covered — no changes. |
