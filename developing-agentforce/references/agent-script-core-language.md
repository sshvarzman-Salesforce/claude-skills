...
> **`is None` vs `== ""` are different checks.** `is None` tests whether a value is *unassigned* (never populated). `== ""` tests whether a string is the *empty string* (assigned, but blank). A `mutable string = ""` default holds `""`, not `None`. Use `== ""` for "has the user given me this defaulted value yet?"; use `is None` for linked/optional values that may never be populated.
...
- **Grouping with `( )`**: When you combine `and` and `or` in one condition, wrap groups in parentheses to make evaluation order explicit — mixed `and`/`or` precedence is easy to get wrong. `if (@variables.status == "pending" or @variables.status == "review") and @variables.verified == True:`. This applies equally in `if` conditions and in `available when` filters.
...
Lists are declared with `list[T]` and default to `[]`:

```agentscript
variables:
    question_queue: mutable list[string] = []
    current_index: mutable number = 0
```

- **Index access is 0-based**: `@variables.question_queue[0]` is the first item. Valid in logic, `available when`, and (with braces) prompt text: `{!@variables.question_queue[@variables.current_index]}`.
- **Length**: `len(@variables.question_queue)` returns the item count. Works in conditions, `available when`, and prompt text.
- **No `for` loop.** Agent Script cannot iterate a list in a single pass. To walk a list, pair it with an index variable and advance the index once per turn:

```agentscript
reasoning:
    instructions: ->
        if @variables.current_index < len(@variables.question_queue):
            | Ask the customer: {!@variables.question_queue[@variables.current_index]}
        # after the answer is captured, advance to the next item
        set @variables.current_index = @variables.current_index + 1
        if @variables.current_index >= len(@variables.question_queue):
            transition to @subagent.summary
```

- **Lists cannot be slot-filled with `...`** — the LLM fills scalar inputs, not collections. Populate a list from an action/Flow output that returns a collection (`set @variables.records = @outputs.records`), then walk it with an index as above.
...
Slot-filling (`...`) only works for **top-level inputs of LLM-exposed reasoning actions** (actions under `reasoning.actions` that the LLM chooses to call). Inputs of actions invoked deterministically with `run`, and inputs of chained/dependent actions, must be bound to `@variables` or literals — they resolve before the LLM reasons, so there is nothing to slot-fill from.
