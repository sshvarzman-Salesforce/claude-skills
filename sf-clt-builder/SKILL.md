...
> ⚠️ **The action MUST expose at least ONE output the planner can read
> (`isUsedByPlanner: true`), or it apologizes even though the card renders.** This
> is the single most confusing CLT failure and the table above walks you straight
> into it: the card is `isUsedByPlanner: false` (correct), and if you *also* leave
> `success`/`errorMessage` at `false`/`false` (as the last row shows) **and there is
> no chaining passthrough, every single output is filtered from the planner.** The
> action then returns `success:true` with a fully-populated card in the raw output
> inspector, the LWC is fine, the Lightning Type binding is fine — but the planner
> gets back nothing it is allowed to see, concludes the action failed, and narrates
> *"It seems I couldn't retrieve the details…"* instead of rendering. It looks like
> a render bug; it is a **planner-visibility** bug.
>
> A **list/multi-item** action usually escapes this by accident because it carries a
> natural count passthrough (`accountCount`, `isUsedByPlanner: true`) — which is why
> the list card renders while the byte-identical **single-detail** card apologizes.
>
> **Fix:** flip `success` to `isUsedByPlanner: true` (cheapest), and/or add one
> positive-confirmation passthrough scalar the planner can read. Verified on the
> Questrade Trading SRA: the *only* change that stopped the apology was giving the
> detail action a planner-visible `success` + an `accountName` passthrough — no LWC,
> schema-binding, or Lightning Type change was involved. So when *only* the card is
> declared, override the last row of the table above to `success: isDisplayable
> false / isUsedByPlanner **true**`.
...
| **EVERY output `isUsedByPlanner: false`** (card filtered + `success`/`errorMessage` at `false`/`false`, no passthrough) | Planner sees nothing, apologizes ("couldn't retrieve…") even though the card renders and `success:true` shows in the raw inspector — looks like a render bug, is a visibility bug | Keep at least one planner-visible output: flip `success` to `isUsedByPlanner: true` and/or add a positive-confirmation passthrough scalar. Single-detail actions hit this; list actions escape via their count passthrough |
...
| 2026-09-23 | **Planner-visibility apology fix.** Documented the "every output filtered → planner apologizes even though the card renders" failure, verified on the Questrade Trading SRA. A single-detail action whose only outputs are the card (`isUsedByPlanner: false`) plus `success`/`errorMessage` at `false`/`false` leaves the planner blind; it returns `success:true` with a full card in the raw inspector yet narrates "It seems I couldn't retrieve the details." List actions escape via their count passthrough. Fix: keep ≥1 planner-visible output — flip `success` to `isUsedByPlanner: true` and/or add a positive-confirmation passthrough. Added warning to the Display-flags section and a Common Pitfalls row. |
